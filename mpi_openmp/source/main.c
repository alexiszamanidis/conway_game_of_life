...
int main( int argc, char **argv ) {
    ...
    int number_of_processes, rank_of_the_process, generation, i, j, provided;
...
//    print_neighbour_ranks(neighbour_processes,rank_of_the_process);
    ...
    for( generation = 0 ; generation < arguments.generations ; generation++ ) {
        ...
        #pragma omp parallel for shared (current_generation, next_generation) private(i, j) reduction(+:neighbours) collapse(2)
    ...
    //    print_grid_side_dimensions(grid_side_dimensions,current_generation->dimension,rank_of_the_process);
        ...
        omp_set_num_threads(arguments.threads);
        #pragma omp parallel for shared (current_generation, next_generation, grid_side_dimensions) private(i) reduction(+:neighbours)
